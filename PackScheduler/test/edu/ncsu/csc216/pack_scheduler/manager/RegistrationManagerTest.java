...
/**
 * The test class for the RegistrationManager object. The class tests: 
 * retrieving the course catalog, student directory, login and logout, and retrieving the current User.
 * 
...
public class RegistrationManagerTest {
	...
	public void testGetCourseCatalog() {
		//reset singleton with beforeEach setUp()
		//empty course catalog array
		assertNull(manager.getCourseCatalog().getCourseFromCatalog("", ""));
	...
	public void testGetStudentDirectory() {
		assertNull(manager.getStudentDirectory().getStudentById("0"));
	...
	public void testLogin() {
		//Test login by using the registrar.properities data containing the password, which is secret
		Properties prop = new Properties();
		
		try (InputStream input = new FileInputStream(PROP_FILE)) {
			prop.load(input);
		} catch (IOException e) {
			fail("Cannot open " + PROP_FILE + " for testing.");
		}
		// Non-existent user
		Exception noUserException = assertThrows(IllegalArgumentException.class, () -> manager.login("wrongid", "wrongpw"));
		assertEquals("User doesn't exist.", noUserException.getMessage());
		
		// Wrong password
		assertFalse(manager.login(prop.getProperty("id"), "wrongpw"));
		
		// Correct id and password
		assertTrue(manager.login(prop.getProperty("id"), prop.getProperty("pw")));
		
		// Log in when current User logged in
	...
	public void testGetCurrentUser() {
		//when manager is created, fields should be same as properties file
		Properties prop = new Properties();
	...
	public void testAddFacultyToCourse() {
		...
		Faculty f = new Faculty("Bob", "Jenkins", "bjenk", "bjenk@ncsu.edu", "pw", 2);
		
		assertFalse(manager.addFacultyToCourse(null, f));
		
		manager.login(registrarUsername, registrarPassword);
		
		Course c = new Course("CSC216", "Software", "001", 3, "sesmith5", 40, "MW");
		
		Exception e1 = assertThrows(IllegalArgumentException.class,
				() -> manager.addFacultyToCourse(c, f));
		assertEquals("Illegal Action", e1.getMessage());

		Faculty f2 = new Faculty("Sarah", "Heckman", "sesmith5", "sesmith5@ncsu.edu", "pw", 2);
		...
		assertTrue(manager.addFacultyToCourse(course, f2));
		
	}
	
	/**
	 * Tests removeFacultyToCourse()
	 */
	@Test
	public void testRemoveFacultyToCourse() {
		...
		assertFalse(manager.removeFacultyFromCourse(null, f));
		
		manager.login(registrarUsername, registrarPassword);
		
		Course c = new Course("CSC216", "Software", "001", 3, "smith", 40, "MW");
